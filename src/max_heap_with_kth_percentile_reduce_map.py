...
for line in sys.stdin:
    ...
    if stop - start == EXEMPLAR_SIZE:
        ...
        ordered_ids = [x for _,x in sorted(zip(scores, ids))]
        ordered_scores = [x for x,_ in sorted(zip(scores, ids))]
        ...
        id_of_interest = ordered_ids[min_dist_scores_index]
        score_of_interest = ordered_scores[min_dist_scores_index]
        ...
        new_line = '{}\n'.format('\t'.join([chrom, str(new_start), str(new_stop), id_of_interest, str("{:.5f}".format(score_of_interest)), strand]))
